...
impl Command {
        ...
        write_file(
            ...
            Signed {
                signed: Root {
                    ...
                    keys: HashMap::new(),
                    roles: tough::schema::Roles::new(&hashmap! {
    ...
    async fn remove_key(path: &Path, key_id: &Decoded<Hex>, role: Option<RoleType>) -> Result<()> {
        ...
        if let Some(role) = role {
            if let Some(role_keys) = root.signed.roles.0.get_mut(&role.to_string()) {
                role_keys
                    .keyids
                    .iter()
            ...
            for role_keys in root.signed.roles.0.values_mut() {
                role_keys
                    .keyids
                    .iter()
                    .position(|k| k.eq(key_id))
...
fn add_key(root: &mut Root, role: &[RoleType], key: Key) -> Result<Decoded<Hex>> {
    ...
    for r in role {
        let entry = root.roles.0.entry(r.to_string()).or_insert_with(|| role_keys!());
