...
impl<'a> RepositoryLoader<'a> {
    ...
    pub async fn load_uptane(self) -> Result<UptaneRepository> {
        UptaneRepository::load(self).await
    }    
...
impl UptaneRepository {
    async fn load(loader: RepositoryLoader<'_>) -> Result<UptaneRepository> {
...
async fn load_remote_sessions(
    ...
    let reader = fetch_max_size(
        ...
        "max_timestamp_size argument",
    ).await?.into_vec().await.context(error::TransportSnafu { url: metadata_base_url.clone()})?;

    let remote_sessions: Signed<RemoteSessions> =
        serde_json::from_slice(&reader).context(error::ParseMetadataSnafu {
    ...
    if let Some(Ok(old_remote_sessions)) = datastore
        .bytes("remote-sessions.json").await?
        .map(|v| serde_json::from_slice::<Signed<RemoteSessions>>(&v))
    {
    ...
    if expiration_enforcement == ExpirationEnforcement::Safe {
        check_expired(datastore, &remote_sessions.signed).await?;
    }

    datastore.create("remote-sessions.json", &remote_sessions).await?;
